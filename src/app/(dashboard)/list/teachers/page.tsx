...
return( <tr key={item.id} className='border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight'>
      ...
      <td className="hidden md:table-cell text-sm">{item.subjects.map((sub)=>sub.name).join(",")}</td>
      <td className="hidden md:table-cell text-sm">{item.classes.map((cls)=>cls.name).join(",")} </td>
...
const TeachersList = async ({  searchParams}:{searchParams: {[key:string]:string};
}) => {
